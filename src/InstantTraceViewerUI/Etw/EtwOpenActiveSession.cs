    ...
    internal class OpenActiveSession : IDisposable
    {
        ...
        public void DrawWindow(IUiCommands uiCommands, ref bool showOpenActiveSession)
        {
            ...
            if (ImGui.Begin("Active Sessions (ETW)", ref showOpenActiveSession))
            {
                ...
                {
                    ImGui.TextUnformatted("No active sessions found. Run this application as administrator to see active sessions.");
                }
                        ...
                        if (session.IsRealTime)
                        {
                            if (ImGui.Button("Open real-time"))
                            {
                                try
                                {
                                    ...
                                    uiCommands.AddWindow(new LogViewerWindow(new Etw.EtwTraceSource(logViewerEtwSession, kernelProcessThreadProviderEnabled, logViewerEtwSession.SessionName)));
                        ...
                        if (!string.IsNullOrEmpty(session.FileName))
                        {
                            ...
                            if (ImGui.Button("Open ETL file"))
                            {
                                try
                                {
                                    ...
                                    uiCommands.AddWindow(new LogViewerWindow(Etw.EtwTraceSource.CreateEtlSession(session.FileName)));
                                    showOpenActiveSession = false; // Close this window.
                                }
                                catch (Exception ex)
                                {
