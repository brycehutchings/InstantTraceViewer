...
namespace InstantTraceViewerTests
{
    ...
    public class ParserIdeaTests
    {
        ...
        public void TestParse()
        {
            ...
            ParserIdea conditionParser = new(mockTraceTableSnapshot.Schema);
            ...
            {
                ParserIdea.Result expressionResult = conditionParser.Parse(text);
                //Assert.IsTrue(expressionResult.WasSuccessful, $"Condition {text} did not parse\n{expressionResult}");
                TraceTableRowPredicate compiledFunc = expressionResult.Expression.Compile();
